...
//	FILE:	key_emeter.c 
// 
//	TITLE:	Key Task for 1(row) x 4(column) Matrix
//*******************************************************************************************************
//
...
INT8U     KeyBuf[KEY_BUF_SIZE];     		// 키보드 버퍼                                             
INT8U     KeyBufInIx;               		// 키보드 버퍼에서 다음 번에 저장될 스캔코드의 인덱스      
INT8U     KeyBufOutIx;              		// 키보드 버퍼에서 다음 번에 읽힐 스캔코드의 인덱스        
INT16U    KeyDownTmr;               		// 키가 얼마나 오랫동안 눌려 있는 지를 카운트한다          
INT8U     KeyNRead;                 		// 키보드로부터 읽힌 키의 개수                             

INT8U     KeyRptStartDlyCtr;        		// 자동반복 기능이 시작되기 전의 스캔타임                  
INT8U     KeyRptDlyCtr;             		// 자동반복 기능이 다시 시작되기 전의 스캔타임             

INT8U     KeyScanState;             		// 키보드 스캐닝 함수의 현재 상태                          
...
*                                           키보드 디코드
*
* 설명     : 이 함수는 눌려진 키의 코드를 결정하기 위해 호출된다
* 전달인자 : 없음
* 리턴값   : 키 코드
*********************************************************************************************************
*/
...
INT8U KeyDecode (void)
{
	INT8U   col;
    	...
    	col    = ~(PINB & 0xF0);                        	// 열을 읽는다        
    	                            
    	
    	msk    = 0x10;                                    // 열을 스캔하기 위해서 비트 마스크 함           
    	col_id =    0;                                    // 열의 값을 세트한다 (0..5)                     
    	done   = FALSE;
     while(col_id < KEY_MAX_COLS && !done) {          	// 모든 열을 조사한다                            
        if(col & msk) {                              	// 이번 열이 눌려 있는 지를 확인한다             
           done = TRUE;                             	// 종료, col_id에 키의 열의 값이 저장되어 있음(0..5)  
        } else {
           col_id++;
           msk <<= 1;
        }
    	...
    	return (row * KEY_MAX_COLS + col_id);    		// 스캔코드를 리턴한다                           
}
...
void KeyFlush(void)
{
        ...
        KeyGetKey();                            	// ... 버퍼로 부터 다음 스캔코드를 읽어온다           
    	}
}
...
//	if(row == KEY_ALL_ROWS) {
//        PORTE = 0xF8;                			// 모든 행을 1로 출력함 (SCAN0~4)                            
//    	} else {
//        PORTE = (8 << row);         			// 원하는 행만 1로 출력함                           
//    	} 
//    	Delay_us(5);
}
